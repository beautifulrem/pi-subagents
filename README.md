...
The stable v1 status/result fields are `lifecycleArtifactVersion`, `runId`/`id`, `sessionId`, `mode`, `state`, `stepIndex`, `runnableAt`, `queueDurationMs`, `startedAt`, `lastUpdate`, `endedAt`, `durationMs`, `cwd`, `asyncDir`, `sessionFile`, `outputFile`, `workflowGraph`, `steps`, `results`, `totalTokens`, `totalCost`, `model`/`attemptedModels`/`modelAttempts`, `toolCount`, `turnCount`, and nested `children` when a child is allowed to launch subagents. Async result children use stable flat `stepIndex` values to map back to status steps, including across empty or synthetic dynamic outcomes. On async child steps, `runnableAt` starts only after dependencies, dynamic materialization, and setup finish; `queueDurationMs` is scheduler-capacity wait, remains absent while queued, and freezes when the child starts, pauses, or terminates without starting. Exact and list status always show valid persisted `modelAttempts` count and failures; nonzero, numerically valid usage metrics additionally show turns, input/output, cache read/write, and cost. `events.jsonl` records lifecycle transitions such as `subagent.run.started`, `subagent.step.started`, `subagent.step.completed`/`failed`/`paused`/`stopped`, control attention events, nested interrupt failures, and `subagent.run.completed`/`stopped`; run boundary events include the lifecycle artifact version. Consumers should read these JSON files instead of scraping terminal output; unknown fields and event types should be ignored for forward compatibility.
