...
describe("registered subagent tool description", () => {
	it("keeps full mode safe and free of hardcoded builtin agent names", () => {
		...
		assert.match(description, /Exact-output tasks.*acceptance:false.*every task and dynamic group/i);
	...
	it("offers a compact mode that keeps safety-critical guidance", () => {
		...
		assert.match(description, /exact token\/JSON transforms.*acceptance:false.*every task and dynamic group/i);
