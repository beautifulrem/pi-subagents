	...
	if (asyncDir) {
		...
		if (status) {
			...
			for (const [index, step] of (status.steps ?? []).entries()) {
				...
				lines.push(`${stepLineLabel(status, index)}: ${phase}${display} ${step.status}${modelText}${stepActivityText ? `, ${stepActivityText}` : ""}${steeringSuffix}${acceptanceText}${queueText}${budgetText}${errorText}`);
