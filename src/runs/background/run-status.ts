...
import { formatDuration, formatModelAttemptUsage, formatModelThinking } from "../../shared/formatters.ts";
	...
	if (asyncDir) {
		...
		if (status) {
			...
			for (const [index, step] of (status.steps ?? []).entries()) {
				...
				const usageText = attemptUsage ? `, usage: ${attemptUsage}` : "";
				const queueDurationMs = step.queueDurationMs ?? (step.status === "pending" && step.runnableAt !== undefined ? Math.max(0, (deps.now?.() ?? Date.now()) - step.runnableAt) : undefined);
				const queueText = queueDurationMs !== undefined ? `, queue: ${formatDuration(queueDurationMs)}` : "";
				...
				lines.push(`${stepLineLabel(status, index)}: ${phase}${display} ${step.status}${modelText}${stepActivityText ? `, ${stepActivityText}` : ""}${steeringSuffix}${acceptanceText}${usageText}${queueText}${budgetText}${errorText}`);
