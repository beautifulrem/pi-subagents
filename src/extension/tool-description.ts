...
• Exact-output tasks: when a token/JSON transform must return only the requested output and needs no implementation or review evidence, set acceptance:{level:"none",reason:"exact-output transform"} explicitly. For dynamic expand, put it both on the dynamic step and its parallel template. Never rely on auto-inference to suppress acceptance.
...
• Dynamic expand step: { parallel:{agent:"agent-b",task:"Handle {item}"}, expand:{from:{output:"seed",path:"/items"},item:"item",maxItems:4}, collect:{as:"results"} }. Here parallel must be one template object, not an array, and the step must not also set agent. If the seed uses outputSchema for an exact JSON transform, give that seed acceptance:{level:"none",reason:"exact-output transform"} too.
...
DIAGNOSTICS:
• { action: "doctor" } - read-only report for runtime/source identity, paths, discovery, sessions, and intercom
...
• Dynamic expand step: {parallel:{agent:"agent-b",task:"Handle {item}"},expand:{from:{output:"seed",path:"/items"},item:"item",maxItems:4},collect:{as:"results"}}. parallel is one template object, not an array; do not also set agent on that step. For an exact-JSON outputSchema seed, set the same canonical acceptance none on the seed too.
...
• For exact token/JSON transforms that need no implementation or review evidence, set acceptance:{level:"none",reason:"exact-output transform"} explicitly; for dynamic expand put it both on the step and its parallel template. Do not rely on inference.`;
