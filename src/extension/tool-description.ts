...
• Exact-output tasks: when a token/JSON transform must return only the requested output and needs no implementation or review evidence, set acceptance:false explicitly. For dynamic expand, put it both on the dynamic step and its parallel template. Never rely on auto-inference to suppress acceptance.
...
• For exact token/JSON transforms that need no implementation or review evidence, set acceptance:false explicitly; for dynamic expand put it both on the step and its parallel template. Do not rely on inference.`;
